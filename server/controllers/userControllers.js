...
const updateUserProfile = asyncHandler(async (req, res, next) => {
  ...
  if (user) {
    try {
      ...
      user.following_approved =
        req.body.following_approved || user.following_approved;
      user.following_request_waiting =
        req.body.following_request_waiting || user.following_request_waiting;
...
const getFavorites = asyncHandler(async (req, res, next) => {
  ...
  for (let i = 0; i < sorted_favorites_list.length; i++) {
    const player = await Player.findById(
      mongoose.Types.ObjectId(sorted_favorites_list[i])
    );
...
const deleteFollowingRequests = asyncHandler(async (req, res, next) => {
  ...
  const following_request_waiting = await user.following_request_waiting.filter(
    (following) => {
  ...
  const following_sent = await second_user.following_sent.filter(
    (following) => {
